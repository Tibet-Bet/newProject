...
func (d *db) FindOne(ctx context.Context, id string) (u user.User, err error) {
	...
	if err != nil {
		return u, fmt.Errorf("failed toconvert hex to objectid.hex: %s", id)
...
func (d *db) Update(ctx context.Context, user user.User) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert usr ID to ObjectID. ID=%s", user.ID)
	}
	...
	d.logger.Tracef("Matched %d documents an Modified %d documents", result.MatchedCount, result.ModifiedCount)
	return nil
}
...
func (d *db) Delete(ctx context.Context, id string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to convert usr ID to ObjectID. ID=%s", id)
	}
	filter := bson.M{"_id": objectID}
